...
const order = sortDocuments(docs).map((d) => d.kind === "package" ? d.title.split(" — ")[0] : d.kind);
...
check("the operating agreement follows", order[4] === "Operating Agreement No. 1", order);
check("the rest come after, newest first", order.slice(5).join(",") === "legal_mail,S Corporation Election Package (Form 2553),certificate-of-status", order.slice(5));
