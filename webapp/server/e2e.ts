...
//     and where the client appoints us, the appointment is what stands in for it.
{
  ...
  // named by the Florida document number the office types, signed "/s/".
  {
    ...
    if (clientStmt && hasPdftotext) {
      ...
      check("read off the Statement PDF: signed By: /s/ Caitlin Kirwan over her printed name and title, and item 5 speaks of a member-managed company (15 Sep 2026)", /By: \/s\/ Caitlin Kirwan/.test(text) && /Caitlin Kirwan Manager/.test(text) && /rest with its members, as provided in its operating agreement/.test(text) && !/rests with its manager or managers/.test(text) && /Statement of Authorized Representative of [^—]+ — generated by MyFloridaSeriesLLC/.test(text), text.match(/By: \/s\/[^.]{0,80}|rest[s]? with its [^,]{0,40}/g));
    }
    if (clientStmt) {
      const bytes = new Uint8Array(await (await fetch(`${BASE}/api/portal/documents/${clientStmt.id}/download`, { headers: { Cookie: apPw.cookie } })).arrayBuffer());
      ...
      if (text !== null) {
        ...
        check("read off the PDF: paragraph 6 says the Filer is not the agent and names PS 2 as a separate series", /6\. Registered agent service\. The Filer is not the Company's registered agent\. If the Company has engaged FLORIDA PROTECTED SERIES, LLC - PS 2, a separate protected series of the same limited liability company, as its registered agent/.test(flat) && !/or its affiliate/.test(flat), flat.match(/6\. Registered agent service\.[^7]*/)?.[0]);
        check("read off the PDF: a conformed signature by the named signer, with title and date", /By: \/s\/ (.+?) \1 Manager Date: [A-Z][a-z]+ \d{1,2}, \d{4}/.test(flat), flat.slice(-300));
...
if (mint.status === 200) {
    ...
    { status: consentPdf.status, len: consentBytes.length });
  {
    ...
    if (text !== null) {
      ...
      check("read off the consent PDF: a sole owner's series is managed by the Member, in the one-owner agreement's own words (15 Sep 2026)", /Managed by The Member, as protected-series manager \(ss\. 605\.2304\(1\)-\(2\), 605\.2107\(1\)\(n\), Fla\. Stat\.\)/.test(flat) && !/The Members, as protected-series managers/.test(flat) && !/The Company, as protected-series manager/.test(flat), flat.match(/Managed by[^|]{0,160}/)?.[0]);
      check("read off the consent PDF: titled for the sole member, with the edition footer (15 Sep 2026)", /WRITTEN CONSENT OF THE SOLE MEMBER/.test(flat) && !/OF THE MEMBERS/.test(flat) && /generated by MyFloridaSeriesLLC · Master First Edition/.test(flat) && /same effect as a vote/.test(flat) === false, flat.match(/CONSENT OF THE [A-Z ]+|Master First Edition[^*]{0,30}|same effect as a vote/g));
