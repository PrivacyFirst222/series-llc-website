...
export function assembleOa(inputs: OaInputs): { markdown: string; title: string } {
  ...
  // ---- back matter ----
  // The footer under the last exhibit is the master's own line, not composed
  // here. It is lifted off the end of the file before anything else touches s:
  // the Asset Schedule template runs to end-of-file, so left in place it would
  // ride into every instantiated schedule exactly as the draft colophon once
  // did. Re-appended, filled, after the exhibits.
  const FOOTER_LINE =
  ...
  // append instantiated series exhibits, then the master's footer line
