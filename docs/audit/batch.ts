...
if (cmd === "authorize") {
  ...
  for (const x of parts) {
    ...
    if (part.status !== "open" && part.batch !== b.id) die(`${name(x)} is ${part.status} in batch ${part.batch} — it cannot be claimed twice`);
  ...
  finish(`batch ${b.id} r${b.revision} rejected on Adam's record of ${rec.at.slice(0, 16)} — nothing in it is released; ${parts.length} item(s) back to open`);
